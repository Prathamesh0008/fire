    ...
    <footer className="bg-secondary text-gray-300 pt-12 pb-6">
      <div className="container-custom">
          ...
          <div>
            ...
            <ul className="space-y-1">
              {products.slice(0, 5).map((product, index) => (
                <li key={index}>
                  <Link href={`/products/${product.toLowerCase().replace(/\s+/g, '-')}`} className="text-sm hover:text-primary transition">
                    {product}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-3">
              ...
              <li className="flex items-center gap-3">
                ...
                <span className="text-sm">info@shreejee.com</span>
              </li>
            </ul>
        ...
        <div className="border-t border-gray-700 pt-6 text-center text-sm">
          <p>&copy; {new Date().getFullYear()} Shreejee Technology. All rights reserved.</p>
        </div>
      </div>
    </footer>
  )
